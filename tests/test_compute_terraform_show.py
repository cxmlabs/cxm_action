...
class TestComputeTerraformShow:
    ...
    def test_successful_execution(self, mock_run, tmp_path):
        ...
        terraform_output = {"format_version": "1.0", "terraform_version": "1.5.0", "values": {"root_module": {"resources": []}}}

        mock_run.return_value = subprocess.CompletedProcess(
            args=["terraform", "show", "-json"], returncode=0, stdout=json.dumps(terraform_output), stderr=""
        )
        ...
        mock_run.assert_called_once_with(
            ["terraform", "show", "-json"], cwd=tmp_path, capture_output=True, text=True, check=True, timeout=300
        )
    ...
    def test_terraform_timeout(self, mock_run, tmp_path):
        ...
        mock_run.side_effect = subprocess.TimeoutExpired(cmd=["terraform", "show", "-json"], timeout=300)
